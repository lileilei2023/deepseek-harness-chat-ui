...
**想在会话流里加东西，先看 `tool.call.toolview` 而不是 `conversation.chat.node`。**
前者按**工具名**分发，`subagent` 这样没有上游占用者的键，注册就是新增；后者的
`tool-call` 键归 `client-ui-tool`，接管等于重新实现所有工具的富渲染。占用名单在
`packages/extensions/cordis-client-runner/src/client/slot-catalog.ts` 里，实测比
读文档快。代价是兜底行的「展开看输出」得自己实现。

## 设计立场

- 这是**聊天软件**，不是 IDE。工作台的职责是「拿到团队交付物」，不是复刻编辑器。
- 形状编码类别：肖像是圆（人），房间是圆角方块（群或普通对话）。房间瓷砖 46px、
  联系人圆脸 40px；瓷砖里的脸要把瓷砖填满（两人对角相叠，三四人 2×2），否则一个
  群在列表里会比一个人轻一半。
