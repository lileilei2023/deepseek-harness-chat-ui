...
| 群组 | 成员搜索、协调者、群组职能、项目目录绑定；房间由成员头像组成的方块标识——两人对角相叠，三四人排 2×2 |
...
| 按发言人配脸 | 每段转述带上撰写它的那个成员的头像；协调者派活的那一行也换成成员本人的头像、昵称与任务，而不是一行「工具调用 · subagent」 |
| 看得见谁在干活 | 成员面板列出每位成员在做什么；输入框上方一条「正在工作」，位置与聊天软件的「正在输入」一致；点成员即 @，`@所有人` 让每人各答一次 |
| 群公告 | 房间的长期约定，每次新会话自动带上，不用再重复交代 |
| 工作台 | 项目文件、终端、产物、Diff、浏览器与旁路任务；按房间记住上次打开的面板、展开的目录与文件标签 |
| 终端 | 持续的流而不是一次快照：输出边跑边长，颜色正确，可以停止，有命令历史 |
| 产物 | 来自会话日志里真正写文件的调用，能追到是哪一步、哪位成员产生；可下载、可在新标签打开、可定位到文件夹 |
| 自动化 | 与外壳「新会话」并列的顶层入口，场景模板、Room 绑定、立即运行；运行记录带未读数、失败原因与重试，可开桌面通知 |
...
`TODO.md` 按严重度列出剩下的，每条写清现状、为什么不好用、做到什么算完；文末列出这一轮已经还掉的。眼下最大的两笔是**产物没有版本、看不出两次生成之间改了什么**，以及**对比模式只做到了「让他们各答一次」，结果还不是并排的卡片**。`CLAUDE.md` 给在这个仓库里工作的人：构建、验证与会咬人的地方。
