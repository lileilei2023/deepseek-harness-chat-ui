...
let WorkBuddySkillCatalog = (() => {
	...
	return class WorkBuddySkillCatalog extends _classSuper {
		static {
			...
			__esDecorate(this, null, _roomArtifacts_decorators, {
				kind: "method",
				name: "roomArtifacts",
				static: false,
				private: false,
			...
			__esDecorate(this, null, _readSkillChatTerminal_decorators, {
				kind: "method",
				name: "readSkillChatTerminal",
				static: false,
				private: false,
				access: {
			...
			__esDecorate(this, null, _searchProjectFiles_decorators, {
				kind: "method",
				name: "searchProjectFiles",
				static: false,
				private: false,
				access: {
					has: (obj) => "searchProjectFiles" in obj,
					get: (obj) => obj.searchProjectFiles
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _revealProjectPath_decorators, {
				kind: "method",
				name: "revealProjectPath",
				static: false,
				private: false,
				access: {
					has: (obj) => "revealProjectPath" in obj,
					get: (obj) => obj.revealProjectPath
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		async roomArtifacts(request, signal) {
			signal?.throwIfAborted();
			const workspace = this.ctx.workspaceRegistry.get(WorkspaceId(request.workspaceId));
			if (workspace === void 0) throw new Error("skill-chat: unknown Workspace");
			const root = await realpath(workspace.path);
		...
		*/
		async searchProjectFiles(request, signal) {
			signal?.throwIfAborted();
			...
			const walk = async (directory, depth) => {
				...
				for (const entry of await readdir(directory, { withFileTypes: true }).catch(() => [])) {
					...
					examined += 1;
					if (!request.contents) {
						if (entry.name.toLocaleLowerCase().includes(needle)) hits.push({
							path: full,
							name: entry.name
						});
						continue;
					...
					const info = await stat(full).catch(() => void 0);
					if (info === void 0 || info.size > SEARCH_MAX_FILE_BYTES) continue;
					const text = await readFile(full, "utf8").catch(() => void 0);
					if (text === void 0) continue;
					const line = text.split("\n").find((candidate) => candidate.toLocaleLowerCase().includes(needle));
					if (line !== void 0) hits.push({
						path: full,
						name: entry.name,
		...
		*/
		async revealProjectPath(request, signal) {
			signal?.throwIfAborted();
			const workspace = this.ctx.workspaceRegistry.get(WorkspaceId(request.workspaceId));
			if (workspace === void 0) throw new Error("skill-chat: unknown Workspace");
			const root = await realpath(workspace.path);
			...
			const subprocess = this.ctx.get("subprocess");
			if (subprocess === void 0) throw new Error("skill-chat: this Host has no subprocess service");
...
function mutationPath(name, argsRaw) {
	...
	if (name === "edit") return typeof record.old_string === "string" && record.old_string.length > 0 && typeof record.new_string === "string" && record.old_string !== record.new_string ? text(record.file_path) : null;
	if (name !== "str_replace_editor") return null;
	const path = text(record.path);
	if (path === null) return null;
	if (record.command === "create") return typeof record.file_text === "string" ? path : null;
	if (record.command === "str_replace") return typeof record.old_str === "string" && record.old_str.length > 0 ? path : null;
	if (record.command === "insert") return typeof record.insert_line === "number" && typeof record.new_str === "string" ? path : null;
	return null;
}
/**
...
*/
function assistantOpening(content) {
