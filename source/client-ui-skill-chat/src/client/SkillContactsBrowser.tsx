...
export function SkillContactsBrowser(props: SkillContactsBrowserProps): React.JSX.Element {
  ...
  return <div className={css.root} data-skill-chat-root>
    ...
    {view === 'contacts' ? <><div className={css.searchWrap}><input className={css.search} value={query} onChange={event => { setQuery(event.target.value) }} placeholder={t('searchAll')}/></div><div className={css.subtabs}><button data-active={contactList === 'frequent'} onClick={() => { setContactList('frequent') }}>{t('frequentContacts')}</button><button data-active={contactList === 'all'} onClick={() => { setContactList('all') }}>{t('allContacts')}</button></div><div className={css.modeBar}><span>{t('displayMode')}</span><button type="button" data-active={mode === 'persona'} onClick={() => { setMode('persona') }}>{t('personaMode')}</button><button type="button" data-active={mode === 'raw'} onClick={() => { setMode('raw') }}>{t('rawMode')}</button></div><div className={css.list}>{phase === 'loading' ? <div className={css.status}>{t('loading')}</div> : phase === 'error' ? <div className={css.status}>{t('loadFailed')}</div> : visibleContacts.length === 0 ? <div className={css.status}>{t('searchEmpty')}</div> : visibleContacts.map(contactRow)}{deferredQuery.length >= 2 && externalPhase === 'loading' ? <div className={css.status}>{t('searchingExternal')}</div> : null}{externalResults.map(result => marketplaceRow(result))}</div></>
      ...
      : <><div className={css.quickActions}><button className={css.generalChatButton} type="button" onClick={() => { void beginGeneralChat() }}><span className={css.generalAvatar}>✦</span><span><strong>开始普通对话</strong><small>不启用 Skill，直接与模型交流</small></span><b>＋</b></button></div><div className={css.historySection}><button className={css.historyToggle} type="button" aria-expanded={historyExpanded} onClick={() => { setHistoryExpanded(current => !current) }}><span><strong>最近对话</strong><small>{recentSessions.length} 条</small></span><b>{historyExpanded ? '⌃' : '⌄'}</b></button>{historyExpanded ? <div className={css.historyList}>{recentSessions.length === 0 ? <EmptyState className={css.emptyHistory} title="暂无会话">先开始一段普通对话或 Skill 对话。</EmptyState> : recentSessions.map(item => { const room = visibleRooms.find(candidate => candidate.roomId === item.roomId); if (room === undefined) return null; return <button className={css.historyRow} data-current={item.harnessSessionId === currentSessionId} type="button" key={item.roomSessionId} onClick={() => { openHistorySession(room, item) }}>{roomAvatar(room, true)}<span><strong>{item.title}</strong><small>{room.title} · {new Date(item.updatedAt).toLocaleString()}</small></span></button> })}</div> : null}</div><div className={css.sectionHeading}><div><strong>群组</strong><small>{groupRooms.length} 个协作空间</small></div><button type="button" onClick={openGroupCreator}>＋ 新建</button></div><div className={css.groupRoomList}>{groupRooms.length === 0 ? <p className={css.sectionHint}>把常用 Skill 组织成固定协作空间。</p> : groupRooms.map(roomRow)}</div></>}
