...
import { Avatar, AvatarStack, Button, ChatBubble, Dialog, Drawer, EmptyState, IconButton, RoomRow } from './ui/index.tsx'
...
import './theme.css'
import { hashOf, roomTime } from './format.ts'
import { WorkbenchDrawer } from './workbench/index.tsx'
import type {
  ArtifactVersion, ProjectDirectoryListing, ProjectEntry, ProjectFilePreview, ProjectToolKind,
  RoomArtifact, TerminalSnapshot,
 ...
 */
export function scheduleLabel(rule: string): string {
...
export function SkillContactsBrowser(props: SkillContactsBrowserProps): React.JSX.Element {
  ...
  // How a recurring automation repeats. `interval` counts from the previous run
  // and therefore drifts; the clock forms are anchored to a wall clock, which is
  // what "every morning at nine" means.
  const [automationRepeat, setAutomationRepeat] = useState<'daily' | 'weekdays' | 'interval'>('daily')
  const [automationAt, setAutomationAt] = useState('09:00')
  ...
  const createAutomation = async (): Promise<void> => {
    ...
    try {
      await saveState(next, new AbortController().signal)
      setAutomationOpen(false); setAutomationName(''); setAutomationPrompt(''); setAutomationWhen('')
      setAutomationSchedule('once'); setAutomationInterval('1'); setAutomationUnit('d')
      setAutomationRepeat('daily'); setAutomationAt('09:00')
  ...
  return <div className={css.root} data-skill-chat-root>
    ...
    {/* `polite` so an announcement waits for a pause rather than cutting the
      * reader off mid-sentence, and `atomic=false` so only the new text is
      * read. This is where every action in the product reports back, and a
      * screen reader heard none of it. */}
    <div className={css.noticeRegion} aria-live="polite" aria-atomic="false">
      {notice !== null && notice !== '' ? <button className={css.notice} type="button" onClick={() => { setNotice(null) }}>{notice} ×</button> : null}
    </div>
...
export { parseDiff } from './workbench/diff.ts'
export {
  filePreview, ProjectFileView, TerminalPane, WorkbenchDrawer,
} from './workbench/index.tsx'
export type {
  ArtifactVersion, DiffLine, ProjectFilePreview, TerminalPaneProps, WorkbenchDrawerProps,
} from './workbench/types.ts'
