...
function AnimalAvatar({ avatarId, label, small = false }: { readonly avatarId: string; readonly label: string; readonly small?: boolean }): React.JSX.Element {
  return <Avatar avatarId={avatarId} label={label} size={small ? 30 : 42}/>
}
  ...
  return <Drawer className={css.workbenchDrawer} label={title} onClose={props.onClose}>
    <WorkbenchPanel>
      ...
      {props.tool === 'terminal' || props.tool === 'diff' ? <div className={css.terminalWorkbench}>
        ...
        {props.tool === 'terminal' ? <form className={css.terminalComposer} onSubmit={event => { event.preventDefault(); props.onTerminalSubmit() }}><span>$</span><input value={props.terminalCommand} onChange={event => { props.onTerminalCommand(event.target.value) }} placeholder="输入命令，例如 pnpm test" autoFocus/><button type="submit" disabled={props.terminalBusy || props.terminal === null}>运行</button></form> : <div className={css.workbenchFootnote}>显示当前项目的真实 `git diff` 输出。</div>}
      </div> : null}
...
function SidecarDrawer(props: SidecarDrawerProps): React.JSX.Element {
  return <Drawer className={css.sidecarDrawer} label="临时对话" onClose={props.onClose}>
    ...
    <form className={css.sidecarComposer} onSubmit={event => { event.preventDefault(); props.onSubmit() }}><textarea value={props.draft} onChange={event => { props.onDraft(event.target.value) }} placeholder="在当前上下文旁边继续问…"/><Button variant="primary" type="submit" disabled={props.busy || props.draft.trim() === ''}>发送</Button></form>
  </Drawer>
}
...
export function SkillContactsBrowser(props: SkillContactsBrowserProps): React.JSX.Element {
  ...
  const groupRooms = useMemo(() => visibleRooms.filter(room => room.type === 'group'), [visibleRooms])
  const recentSessions = useMemo(() => state.roomSessions
    .filter(session => session.archivedAt === undefined && visibleRooms.some(room => room.roomId === session.roomId))
    .sort((left, right) => right.updatedAt - left.updatedAt), [state.roomSessions, visibleRooms])
  ...
  const roomRow = (room: ChatRoom): React.JSX.Element => {
    ...
    const hover = <div className={css.hoverProfile}><strong>{room.title}</strong><span>{room.systemPrompt?.trim() || '固定 Skill 团队协作空间；未指定 @ 时由协调者处理。'}</span><small>{room.memberIds.length} 名成员 · {coordinator === undefined ? '未设置协调者' : `${displayOf(coordinator, 'persona', state.personas).name} 协调`}</small><small>项目：{linked.join('、') || '未绑定'}</small></div>
    return <HoverCard key={room.roomId} anchor={<RoomRow className={css.roomRow} selected={activeRoom?.roomId === room.roomId} onClick={() => { void openRoom(room) }}><span className={css.avatarStatusWrap}>{roomAvatar(room)}{unread > 0 ? <span className={css.unreadBadge}>{unread > 99 ? '99+' : unread}</span> : null}</span><span className={css.copy}><span className={css.nameLine}><span className={css.name}>{room.title}</span><span className={css.source}>群组 · {room.memberIds.length} 人</span></span><span className={css.description} data-running={running || undefined}>{running ? '正在输入中…' : `${room.sessionIds.length} 个会话 · ${linked[0] ?? '未绑定项目'}`}</span></span><span className={css.time}>{new Date(room.updatedAt).toLocaleDateString()}</span></RoomRow>} content={hover} copyLabel="复制群组信息" copiedLabel="已复制"/>
  }
  ...
  const contactRow = (contact: SkillContact): React.JSX.Element => {
    ...
    return <HoverCard key={contact.id} anchor={<button className={css.row} type="button" onClick={() => { selectContact(contact) }}><AnimalAvatar avatarId={display.avatar} label={display.name}/><span className={css.copy}><span className={css.nameLine}><span className={css.name}>{display.name}</span><span className={css.source} data-source={contact.source}>{contact.source === 'harness' ? t('sourceHarnessShort') : contact.source === 'workbuddy' ? t('sourceWorkBuddyShort') : 'skills.sh'}</span></span><span className={css.description}>{mode === 'persona' ? contact.name : contact.description}</span></span>{favorites.includes(contact.id) ? <span className={css.favoriteMark}>★</span> : null}</button>} content={<div className={css.hoverProfile}><strong>{display.name}</strong><span>{contact.description}</span><small>原始 Skill：{contact.name} · {contact.sourceLabel}</small>{homepage === undefined ? null : <a href={homepage} target="_blank" rel="noreferrer">查看主页 ↗</a>}</div>} copyLabel="复制 Skill 信息" copiedLabel="已复制" copyText={homepage}/>
  }

  return <div className={css.root} data-skill-chat-root>
    <div className={css.workspaceSection}><div className={css.workspaceLabel}>{t('projectSpace')}</div><div className={css.workspacePicker}><button className={css.workspaceTrigger} type="button" aria-expanded={workspaceOpen} onClick={() => { setWorkspaceOpen(current => !current) }}><span className={css.workspaceIcon}>⌂</span><span>{currentWorkspace?.title ?? t('noWorkspace')}</span><span className={css.chevron}>⌄</span></button><button className={css.workspaceAdd} type="button" aria-label={t('addWorkspace')} onClick={() => { void createWorkspace() }}>＋</button>{workspaceOpen ? <div className={css.workspaceMenu}>{workspaces.items.map(workspace => <button type="button" data-active={workspace.workspaceId === workspaceId} key={workspace.workspaceId} onClick={() => { setWorkspaceId(workspace.workspaceId); setWorkspaceOpen(false) }}><span>⌂</span><strong>{workspace.title}</strong>{workspace.workspaceId === workspaceId ? <b>✓</b> : null}</button>)}</div> : null}</div></div>
    <div className={css.topbar}><div className={css.tabs} role="tablist">{(['chats', 'contacts', 'automations'] as const).map(item => <button className={css.tab} data-active={view === item} type="button" role="tab" aria-selected={view === item} onClick={() => { setView(item) }} key={item}>{item === 'automations' ? '自动化' : t(item)}</button>)}</div><button className={css.addGroup} type="button" aria-label={t('newGroup')} onClick={openGroupCreator}>＋</button></div>
    <div className={css.modeBar}><span>{t('displayMode')}</span><button type="button" data-active={mode === 'persona'} onClick={() => { setMode('persona') }}>{t('personaMode')}</button><button type="button" data-active={mode === 'raw'} onClick={() => { setMode('raw') }}>{t('rawMode')}</button></div>
    ...
    {view === 'contacts' ? <><div className={css.searchWrap}><input className={css.search} value={query} onChange={event => { setQuery(event.target.value) }} placeholder={t('searchAll')}/></div><div className={css.subtabs}><button data-active={contactList === 'frequent'} onClick={() => { setContactList('frequent') }}>{t('frequentContacts')}</button><button data-active={contactList === 'all'} onClick={() => { setContactList('all') }}>{t('allContacts')}</button></div><div className={css.list}>{phase === 'loading' ? <div className={css.status}>{t('loading')}</div> : phase === 'error' ? <div className={css.status}>{t('loadFailed')}</div> : visibleContacts.length === 0 ? <div className={css.status}>{t('searchEmpty')}</div> : visibleContacts.map(contactRow)}{deferredQuery.length >= 2 && externalPhase === 'loading' ? <div className={css.status}>{t('searchingExternal')}</div> : null}{externalResults.map(result => marketplaceRow(result))}</div></>
      ...
      : <><div className={css.quickActions}><button className={css.generalChatButton} type="button" onClick={() => { void beginGeneralChat() }}><span className={css.generalAvatar}>✦</span><span><strong>开始普通对话</strong><small>不启用 Skill，直接与模型交流</small></span><b>＋</b></button></div><div className={css.sectionHeading}><div><strong>群组</strong><small>{groupRooms.length} 个协作空间</small></div><button type="button" onClick={openGroupCreator}>＋ 新建</button></div><div className={css.groupRoomList}>{groupRooms.length === 0 ? <EmptyState className={css.emptyCard} title="还没有群组">创建群组，把常用 Skill 组织成固定协作空间。</EmptyState> : groupRooms.map(roomRow)}</div><div className={css.historySection}><button className={css.historyToggle} type="button" aria-expanded={historyExpanded} onClick={() => { setHistoryExpanded(current => !current) }}><span><strong>会话历史</strong><small>{recentSessions.length} 条</small></span><b>{historyExpanded ? '⌃' : '⌄'}</b></button>{historyExpanded ? <div className={css.historyList}>{recentSessions.length === 0 ? <EmptyState className={css.emptyHistory} title="暂无会话">先开始一段普通对话或 Skill 对话。</EmptyState> : recentSessions.map(item => { const room = visibleRooms.find(candidate => candidate.roomId === item.roomId); if (room === undefined) return null; return <button className={css.historyRow} data-current={item.harnessSessionId === currentSessionId} type="button" key={item.roomSessionId} onClick={() => { openHistorySession(room, item) }}>{roomAvatar(room, true)}<span><strong>{item.title}</strong><small>{room.title} · {new Date(item.updatedAt).toLocaleString()}</small></span></button> })}</div> : null}</div></>}
    ...
    {groupOpen ? <Dialog className={css.groupDialog} label={t('newGroup')} onClose={() => { setGroupOpen(false) }}>
        ...
        <div className={css.groupFormGrid}>
          <div className={css.groupIdentityEditor}><GroupAvatar avatarId={groupAvatar} label={groupName || '新群组'}/><div><strong>群组头像</strong><small>独立圆形标识，与成员 Skill 清晰区分</small></div></div>
          <div className={css.groupAvatarLibrary}>{ANIMAL_AVATARS.map(avatarId => <button type="button" data-selected={groupAvatar === avatarId} key={avatarId} onClick={() => { setGroupAvatar(avatarId) }}><GroupAvatar avatarId={avatarId} label={avatarId} small/></button>)}</div>
          <label className={css.field}><span>{t('groupName')}</span><input value={groupName} onChange={event => { setGroupName(event.target.value) }} placeholder={t('groupNamePlaceholder')}/></label>
        ...
        <div className={css.memberToolbar}><div><strong>选择成员</strong><small>已选 {groupMembers.length} 个，点击成员可加入或剔出</small></div><input value={memberQuery} onChange={event => { setMemberQuery(event.target.value) }} placeholder="搜索昵称、原始 Skill、能力或 skills.sh"/></div>
        ...
        <div className={css.groupFooter}><Button className={css.secondary} onClick={() => { setGroupOpen(false) }}>{t('cancel')}</Button><Button className={css.create} variant="primary" disabled={groupMembers.length < 2 || workspaceId === undefined} onClick={createGroup}>{t('create')}</Button></div>
    </Dialog> : null}
